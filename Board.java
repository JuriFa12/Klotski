...
public class Board 
{
   ...
   //Tommaso
    ...
    public boolean isAvaible(int x,int y) {
    	if(findPieceIndex(x,y)==-1) return true;
        return false;
    }
    public void selectPiece(int x, int y) {
    	int i= findPieceIndex(x,y);
    	if(i!=-1)selected=pieces[i];
    	
    }
    
    public boolean isOutOfBounds(int x, int y, int a, int l) {
    	if(x>5||x<1||y>4||y<1)return true;
    	if(a==1 && l==1)return false;
    	if(l==2) {
    		if(y==4)return true;
        }
    	if(a==2) {
    ...
    public boolean movePiece(int direction) { //ritorna true se la mossa è stata effettuata
    	if(selected==null) {return false;} //Errore
    	int directions[] = selected.getDimensions();
    	if(direction==0) {// verso l'alto
    		if(isOutOfBounds(directions[0]-1,directions[1],directions[2],directions[3])||!isAvaible(directions[0]-1,directions[1])) return false;
    		if(directions[2]==2&&!isAvaible(directions[0]-1,directions[1]+1)) return false;
    	...
    	if(direction==2) { //verso sinistra
    		if(isOutOfBounds(directions[0],directions[1]-1,directions[2],directions[3])||!isAvaible(directions[0],directions[1]-1)) return false;
    		if(directions[3]==2&&!isAvaible(directions[0]-1,directions[1]-1)) return false;
    		selected.move(directions[0], directions[1]-1);
    		
    	}
    	if(direction==3) { //verso destra
    		if(isOutOfBounds(directions[0],directions[1]+1,directions[2],directions[3])||!isAvaible(directions[0],directions[1]+1)) return false;
    		if(directions[3]==2&&!isAvaible(directions[0]-1,directions[1]+1)) return false;
    		selected.move(directions[0], directions[1]+1);
    	}
